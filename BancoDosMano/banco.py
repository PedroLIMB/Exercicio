    ...
    def criar_conta(self):
        ...
        if titular and valor_deposito:  
            try:
                ...
                if valor_deposito > 0:
                    self.app.banco.cursor.execute("SELECT nome FROM banco WHERE nome = ?", (titular,))
                    conta_existente = self.app.banco.cursor.fetchone()
                    if conta_existente:
                        messagebox.showinfo("Conta Existente", f"A conta para {titular} já existe.")
                        self.conta_veri = True
                        self.botao_depositar['state'] = 'normal'
                        self.botao_sacar['state'] = 'normal'
                    else:
